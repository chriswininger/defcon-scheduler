...
const source = 'https://www.wallofsheep.com/blogs/news/list-of-packet-hacking-village-talks-at-def-con-26-finalized'

axios.get(source)
    .then(resp => {
      const jsonTalks = processPage(resp.data)

      fs.writeFileSync(__dirname + '/../public/talksSheep.json', JSON.stringify(jsonTalks, null, 4), 'utf8')
    })
    .catch((err) => {
  ...
  $('tr')
      .each((ndx, el) => {
        ...
        $(el).children('td').each((ndxChild, child) => {
          ...
          talk.title = $(child).text().trim().replace(talk.speaker, '') + ' -- ' + numTalks + ' -- row: ' + ndx + ' -- ' + numTalks
